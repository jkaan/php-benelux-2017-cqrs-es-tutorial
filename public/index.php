...
call_user_func(function () {
    ...
    $app->post('/checkin/{buildingId}', function (Request $request, Response $response) use ($sm) : Response {
        ...
        $commandBus->dispatch(Command\CheckInUser::fromUsername($request->getParsedBody()['username'], $buildingId));

        return $response->withAddedHeader('Location', '/building/' . (string) $buildingId);
    });
    ...
    $app->post('/checkout/{buildingId}', function (Request $request, Response $response) use ($sm) : Response {
        ...
        $commandBus->dispatch(Command\CheckOutUser::fromUsername($request->getParsedBody()['username'], $buildingId));
